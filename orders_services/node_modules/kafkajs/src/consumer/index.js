const Long = require('kafkajs/src/utils/long')
const createRetry = require('kafkajs/src/retry')
const { initialRetryTime } = require('kafkajs/src/retry/defaults')
const ConsumerGroup = require('kafkajs/src/consumer/consumerGroup')
const Runner = require('kafkajs/src/consumer/runner')
const { events, wrap: wrapEvent, unwrap: unwrapEvent } = require('kafkajs/src/consumer/instrumentationEvents')
const InstrumentationEventEmitter = require('kafkajs/src/instrumentation/emitter')
const { KafkaJSNonRetriableError } = require('kafkajs/src/errors')
const { roundRobin } = require('kafkajs/src/consumer/assigners')
...
const sharedPromiseTo = require('kafkajs/src/utils/sharedPromiseTo')
 ...
 * @param {import('kafkajs').PartitionAssigner[]} [params.partitionAssigners]
...
module.exports = ({
  ...
  /** @type {import("kafkajs").Consumer["connect"]} */
  const connect = async () => {
    await cluster.connect()
    instrumentationEmitter.emit(CONNECT)
  }

  /** @type {import("kafkajs").Consumer["disconnect"]} */
  ...
  /** @type {import("kafkajs").Consumer["stop"]} */
  ...
  /** @type {import("kafkajs").Consumer["subscribe"]} */
  ...
  /** @type {import("kafkajs").Consumer["run"]} */
  const run = async ({
    autoCommit = true,
  ...
  /** @type {import("kafkajs").Consumer["on"]} */
  ...
  /**
   * @type {import("kafkajs").Consumer["commitOffsets"]}
  ...
  /** @type {import("kafkajs").Consumer["seek"]} */
  ...
  /** @type {import("kafkajs").Consumer["describeGroup"]} */
  ...
  /**
   * @type {import("kafkajs").Consumer["pause"]}
   * @param topicPartitions
   ...
   *
   * @type {import("kafkajs").Consumer["paused"]}
   */
  const paused = () => {
    if (!consumerGroup) {
      return []
  ...
  /**
   * @type {import("kafkajs").Consumer["resume"]}
   * @param topicPartitions
   *  Example: [{ topic: 'topic-name', partitions: [1, 2] }]
   */
